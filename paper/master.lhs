...
We describe grammar transformations by pattern-matching on rules.
The corresponding pattern synonyms are generated by templates.
The grammar transformation language is a series of blocks, each
made of 3 parts: pattern for input rules, the output rules, and
...
\noindent Grammar transformation language. Note the explicit name
binding. Each begin-end block is applied to the input grammar
exhaustively. Name binding ensures that the metavariable |U| is
always matched to the same nonterminal as sub-blocks are matched
multiple times, generating fresh names each time for |X|, |Y|
etc. The function |collapse| is a user-defined function between
strings; it determines how the constructors of the output
datatype |A| are named.

Each begin-end block should have zero or more name bindings, zero
or one in-block, zero or one one out-block, and zero or more
pattern-blocks. If a grammar transformation is defined without
any pattern-blocks, then the relation between the two datatypes
is empty, and the conversion procedure should always fail.
...
\smallbreak\noindent Definition of the label name
generator |collapse|
\smallskip

\begin{code}
collapse :: String -> String -> String
collapse stem tail =  head (splitOn '_' stem) ++ "_" ++
                      last (splitOn '_' tail)

-- split string on character |c|
-- http://stackoverflow.com/a/4981265
...
< pattern C_1 x y z  =  A_1 x y z
< pattern C_2 x      =  x :: A
< pattern S_3 x y z  =  A_3 x y z
< pattern S_4 x      =  x :: A
< pattern F_5 x      =  A_5 x
< pattern F_6 x y z  =  y :: A
...
<      |  Z_2 Y
< pattern X = Z
<   X_1 (Z_1 y_1 z)  y_n  =  Z_1 y_1 (X_1 z y_n)
<   X_1 (Z_2 y_1)    y_2  =  Z_1 y_1 (X_2 y_2)
<   X_2 y                 =  Z_2 y
< end

\smallbreak\noindent Input grammar\smallskip

< S  =  S_1 S F
<    |  S_2 F

\smallbreak\noindent Output grammar\smallskip

< S'  =  S_1prime F S'
<     |  S_2prime F

\smallbreak\noindent Pattern synonyms\smallskip

< pattern S_1 (S_1prime y_1 z)  y_n  =  S_1prime y_1 (S_1 z y_n)
< pattern S_1 (S_2prime y_1)    y_2  =  S_1prime y_1 (S_2 y_2)
< pattern S_2 y                      =  S_2prime y
